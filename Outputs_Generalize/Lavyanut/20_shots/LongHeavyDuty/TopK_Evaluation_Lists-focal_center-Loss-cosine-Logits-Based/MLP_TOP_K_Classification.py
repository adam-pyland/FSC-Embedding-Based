...
TARGET_EVAL_CLASS = 'LongHeavyDuty'
...
try:
    ...
    print(f"Successfully backed up the running script to: {OUTPUT_DIR}")
except Exception as e:
    print(f"Warning: Could not backup the script file. Reason: {e}")
...
# --- PLOTTING FUNCTION 2: GROUPED BAR CHARTS FOR BOSS ---
...
# --- PLOTTING FUNCTION 3: 100% STACKED BAR CHART ---
def plot_100_percent_stacked_bar(df_sorted, target_class, output_dir):
    total_k = len(df_sorted)
    
    # Select K milestones
    potential_milestones = [10, 25, 50, 100, 150, 300, 400, 500, 750, 1000, 1500]
    selected_ks = [k for k in potential_milestones if k <= total_k]
    if total_k not in selected_ks:
        selected_ks.append(total_k)
    ...
    df_sampled['TP_pct'] = (df_sampled['Cum_TP_obj'] / df_sampled['K']) * 100
    df_sampled['FP_pct'] = (df_sampled['Cum_FP_obj'] / df_sampled['K']) * 100
    
    # Generate the 3-column CSV: TPs, FPs, Total Objects
    csv_df = df_sampled[['Cum_TP_obj', 'Cum_FP_obj', 'K']].copy()
    csv_df.columns = ['TPs', 'FPs', 'Total_Objects']
    csv_path = os.path.join(output_dir, f"{target_class}_TP_FP_Total_Stats.csv")
    csv_df.to_csv(csv_path, index=False)
    print(f"Saved True Positives / False Positives / Total Objects CSV to: {csv_path}")
    ...
    # Stacked Bars (100% normalized)
    ax.bar(x, df_sampled['TP_pct'], width, label='True Positives', color='forestgreen', edgecolor='white', linewidth=0.5)
    ax.bar(x, df_sampled['FP_pct'], width, bottom=df_sampled['TP_pct'], label='False Positives', color='lightcoral', edgecolor='white', linewidth=0.5)
    
    # Formatting Axes
    labels_x = [f"Top {int(k)}" for k in df_sampled['K']]
    ax.set_xticks(x)
    ax.set_xticklabels(labels_x, rotation=45, fontsize=10)
    ax.set_ylabel("Proportion / Percentage (%)", fontsize=11)
    ax.set_ylim(0, 100)
    
    # Title
    first_k = int(df_sampled['K'].iloc[0])
    first_tp = int(df_sampled['Cum_TP_obj'].iloc[0])
    title = (f"1. Cumulative Objects Evaluated\n"
             f"Example: When we go through {first_k} total objects,\n"
             f"we get the first {first_tp} true objects.")
    ax.set_title(title, fontweight='bold', fontsize=13, pad=15)
    
    # Legend
    ax.legend(bbox_to_anchor=(1.01, 1), loc='upper left', fontsize=10)
    
    # Add Absolute Value Annotations inside the bars
    for i, (tp_pct, fp_pct, tp_count, fp_count) in enumerate(zip(
        df_sampled['TP_pct'], df_sampled['FP_pct'], 
        df_sampled['Cum_TP_obj'], df_sampled['Cum_FP_obj'])):
        
        # Draw True Positive text (White)
        if tp_pct > 4: # Prevents text from overflowing small slivers
            ax.text(i, tp_pct / 2, f"{int(tp_count)}", ha='center', va='center', 
                    color='white', fontweight='bold', fontsize=10)
        
        # Draw False Positive text (Black)
        if fp_pct > 4:
            ax.text(i, tp_pct + (fp_pct / 2), f"{int(fp_count)}", ha='center', va='center', 
                    color='black', fontweight='bold', fontsize=10)
                    
    plt.tight_layout()
    plot_path = os.path.join(output_dir, f"{target_class}_100_Percent_Stacked_Bar.png")
    plt.savefig(plot_path, dpi=300)
    print(f"Saved 100% Stacked Bar Chart to: {plot_path}")
...
def main():
    ...
    # NEW FUNCTION ADDED
    plot_100_percent_stacked_bar(df_sorted, TARGET_EVAL_CLASS, OUTPUT_DIR)
